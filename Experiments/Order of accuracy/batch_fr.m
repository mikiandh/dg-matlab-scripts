...
for i = 1:I
    ...
    spmd
        ...
        fprintf(c.Value,'Run\tdt\tDOFs\tK\tp\tg\tWCT\tTV\tL2\tErrorL2\n');
    end
    % Preprocess:
    try
    ...
    try
        parfor j = 1:J
            ...
            mesh = Mesh(FR(runData(j).g,runData(j).p),[-1 1],Periodic(2),runData(j).K);
            solver = SSP_RK3(...
                Burgers,[0 .25],...
            ...
            outputData = {runData(j).Run runData(j).dt mesh.dofCount runData(j).K runData(j).p runData(j).g solver.wallClockTime norms(:).vals};
            fprintf(c.Value,'%d\t%.12f\t%d\t%d\t%d\t%s\t%.12f\t%.12f\t%.12f\t%.12f\n',outputData{:});
