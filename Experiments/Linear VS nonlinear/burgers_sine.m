...
parfor i = 1:size(bases,1)
    ...
    colNames = compose('%s_%s',['t' norms.string]',string(basisNames));
    tbl{i} = array2table(zeros(nT,numel(colNames)));
    tbl{i}.Properties.VariableNames = colNames(:);
    for basis = bases_cpu
        % Mesh:
        ...
        % Time samples:
        T = linspace(0,10/(pi*targetWavemode),nT); % stop before the solution breaks!
        % Notify start:
        fprintf(1,"Starting %d x %s\nTargeted wavemode: %g\n %g of baseline's well-resolved range\n %g of resolved (Nyquist) range\nBreaking time: %g\n\n",...
            mesh.elementCount,basis.getName,targetWavemode,targetWavemode/resolvingWavemode,targetWavemode/nyquistWavemode,T(end))
        exactSolution = @(t,x) smoothBurgersExact(t,x,@(x) 1+.1*sin(pi*targetWavemode*x));
        % Solver:
        ...
        for j = 1:nT
            ...
            tbl{i}{j,endsWith(tbl{i}.Properties.VariableNames,class(basis))} = [T(j) norms.vals]; % store results
        end
        % Notify end:
...
for i = 1:size(bases,1)
    figure
    plot(tbl{i}.t_DGSEM,tbl{i}.ErrorL2_DGSEM,'^-','DisplayName',bases(i,1).getName)
    hold on
    plot(tbl{i}.t_FR,tbl{i}.ErrorL2_FR,'s-','DisplayName',bases(i,2).getName)
    plot(tbl{i}.t_DGIGA,tbl{i}.ErrorL2_DGIGA,'d-','DisplayName',bases(i,3).getName)
    plot(tbl{i}.t_DGIGA_nodal,tbl{i}.ErrorL2_DGIGA_nodal,'o-','DisplayName',bases(i,4).getName)
    hold off
    legend('Location','Best')
    title(sprintf('Burgers, N_{dofs} ~ %d, J = %d',Ndofs,bases(i,1).basisCount))
end