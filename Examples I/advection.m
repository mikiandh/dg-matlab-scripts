...
mesh = Mesh(DGSEM(2),L,25,'boundaries',Periodic(2));

%% Solver
solver = SSP_RK3(Advection,[0 100],...
    'courantNumber',.1,...
    'limiter',TVB('Sensor',APTVD),...
    'showSensor',true,'showLimiter',true,...
    'norms',Norm(["Mass","L2","ErrorL2"]),...
    'exactSolution',@(t,x) IC_combined(x),'iterSkip',500);
